...
import { Trophy, GraduationCap, LayoutGrid, Settings, DollarSign, FolderGit2 } from 'lucide-react';
...
export const Dock: React.FC = () => {
  ...
  const dockItems = [
    ...
    { id: 'creator', icon: LayoutGrid, label: 'Creator', app: 'creator-studio' }, // mock
    { id: 'system', icon: Settings, label: 'System', app: 'system-tools' }, // mock
  ];

  const handleLaunch = (item: typeof dockItems[0]) => {
    // Generate a new card for this app
    const newCard = {
      id: nanoid(),
      type: 'card' as const,
      ...
      cmfk: { correctness: 0, misconception: 0, fog: 0, knowingness: 0 },
      metadata: {},
      position: { x: window.innerWidth / 2 - 200 + (Math.random() * 40 - 20), y: window.innerHeight / 2 - 200 + (Math.random() * 40 - 20), z: 100 },
  ...
  return (
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50">
      <div className="flex items-end gap-3 px-4 py-3 rounded-3xl glass-panel border-white/15 bg-black/30 backdrop-blur-2xl shadow-2xl">
        {dockItems.map((item) => (
          <DockItem key={item.id} icon={item.icon} label={item.label} onClick={() => handleLaunch(item)} />
        ))}
